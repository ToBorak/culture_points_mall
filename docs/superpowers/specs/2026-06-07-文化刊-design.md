...
2. **定榜原子性**：现有 service 的 `Transaction` 闭包存在"`db` 未传入 repo、跨 repo 调用不共享事务"的现状坑。定榜是低频管理动作，本模块**有意偏离**现有惯例，把事务 `*gorm.DB` 显式传入 repo 方法，确保「写 winner + 发 8 分」真原子。spec 明确标注此偏离。
...
| 风险 | 缓解 |
|------|------|
| 定榜跨 repo 事务（现有惯例坑） | 有意偏离，显式传事务句柄，集成测试验证原子性 |
